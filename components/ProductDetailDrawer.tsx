...
export function ProductDetailDrawer({ dish, onAdd, onClose }: ProductDetailDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      >
        <div className="w-12 h-1 bg-zinc-800 rounded-full mx-auto mt-3 mb-4 shrink-0" />

        <div className="flex-1 overflow-y-auto px-6 pb-24 no-scrollbar">
          <div className="w-full h-52 relative rounded-2xl overflow-hidden mb-5 shadow-lg">
            <Image src={dish.image} alt={dish.name} fill sizes="100vw" className="object-cover" />
          </div>
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-3 border border-white/10 bg-white/5 rounded-xl px-3 py-1.5 shrink-0">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-1 hover:bg-white/5 rounded-lg transition-colors">
                <Minus className="w-4 h-4 text-zinc-500" />
              </button>
              <span className="text-lg font-bold w-5 text-center">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="p-1 hover:bg-white/5 rounded-lg transition-colors">
                <Plus className="w-4 h-4 text-white" />
              </button>
            </div>
          </div>

          <p className="text-zinc-500 text-sm font-light leading-relaxed mb-6">{dish.description}</p>
        </div>
