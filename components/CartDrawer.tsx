...
export function CartDrawer({ items, onUpdateQuantity, onCheckout, onClose }: CartDrawerProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      >
        ...
        <div className="px-8 pb-6 border-b border-white/5">
          ...
          <p className="text-zinc-500 font-light">Confira seus itens selecionados</p>
        </div>

        <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 no-scrollbar">
          {items.length === 0 ? (
            <div className="text-center py-20 text-zinc-600">Seu carrinho está vazio.</div>
          ) : (
            items.map((item) => (
              <div key={item.dish.id} className="flex gap-4 items-center">
                <div className="w-24 h-24 relative rounded-2xl overflow-hidden shrink-0">
                  <Image src={item.dish.image} alt={item.dish.name} fill sizes="96px" className="object-cover" />
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-lg uppercase leading-tight">{item.dish.name}</h3>
                  <p className="text-blue-500 font-bold text-lg">R$ {item.dish.price.toFixed(2)}</p>
                </div>
